...
import com.example.servivet.ui.main.view_model.BusinessBookingViewModel
...
class BusinessBookingFragment : BaseFragment<FragmentBusinessBookingBinding,BusinessBookingViewModel>(R.layout.fragment_business_booking), SoldBookingAdapter.Callback {
    private var tabPosition: TabLayout.Tab?=null
    override val binding: FragmentBusinessBookingBinding by viewBinding(FragmentBusinessBookingBinding::bind)
    override val mViewModel: BusinessBookingViewModel by viewModels()
    var type:Int=0
    lateinit var adapter: SoldBookingAdapter
    private var list = ArrayList<MySoldBooking>()
    var currentPage = 1
    ...
    fun setBookingAdapter(){
        if (list != null && list.isNotEmpty()) {
            adapter = SoldBookingAdapter(requireContext(),type,typeReschdule, ArrayList(),this)
...
//                                mViewModel.list.cle
                                ...
                                if (currentPage == 1 && list.size > 0) {
                                    adapter = SoldBookingAdapter(requireContext(),type,typeReschdule,list,this)
                                    binding.recyclercategry.adapter = adapter
                                } else {

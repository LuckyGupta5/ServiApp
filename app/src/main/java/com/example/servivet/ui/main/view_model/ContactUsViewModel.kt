...
class ContactUsViewModel: BaseViewModel() {
    ...
    fun hitContactUsApi() {
        ...
        viewModelScope.launch {
            try {
                contactUsResponse.postValue(Resource.success(repository.contactUsApi(contactUsRequest)))
            }catch (ex: IOException) {
                ex.printStackTrace()
                contactUsResponse.postValue(Resource.error(StatusCode.STATUS_CODE_INTERNET_VALIDATION, null))
